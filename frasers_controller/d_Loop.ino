...
void loop() {
 ...
 //*NOTE: trying inverting "digitalWrite(..., HIGH)" and "digitalWrite(..., LOW)" if you are seeing unexpected behavior.
  rh_PID.Compute();
  if (rh_output > WINDOW_SIZE-0.5) ih_output = 5; //quickest change 0.5s
  ...
  if(digitalRead(WATER_LEVEL_PIN)) { //if no water in the cup, turn off immersion heater and flash LED as an alert
    ...
    ih_PID.Compute();
    if (ih_output > WINDOW_SIZE-0.5) ih_output = 5; //quickest change 0.5s
  ...
  bh_PID.Compute();
  if (bh_output > WINDOW_SIZE-0.5) ih_output = 5; //quickest change 0.5s
